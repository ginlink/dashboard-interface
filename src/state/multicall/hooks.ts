...
function useCallsData111(calls: (Call | undefined)[], options?: ListenerOptions): CallResult[] {
  const { chainId } = useActiveWeb3React()
  const callResults = useAppSelector((state) => state.multicall.callResults)
  const dispatch = useAppDispatch()
  const serializedCallKeys: string = useMemo(
  ...
  return useMemo(
    () =>
      calls.map<CallResult>((call) => {
        ...
        let data
        if (result?.data != null) {
          console.log(BigNumber.from(result?.data).toString())
        }
        if (result?.data && result?.data !== '0x') {
          data = result.data
        }
...
function toCallState111(
  ...
  if (!callResult) return INVALID_CALL_STATE
  const { addc } = callResult
  if (!addc) return INVALID_CALL_STATE
  // if (valid && !blockNumber) return LOADING_CALL_STATE
  if (!contractInterface || !fragment || !latestBlockNumber) return LOADING_CALL_STATE
  const success = addc
  const syncing = 0 < latestBlockNumber
  let result: any | undefined = undefined
  if (success && addc) {
    const arr = { amountIn: null }
    arr.amountIn = addc.amountOut
    try {
      result = arr
    } catch (error) {
      return {
...
export function useSingleContractMultipleData111(
  contract: Contract | null | undefined,
  methodName: string,
  callInputs: any[],
  options?: ListenerOptions,
  ...
  const calls = useMemo(
    () =>
      contract && fragment && callInputs?.length > 0 && callInputs.every((inputs) => isValidMethodArgs(inputs))
        ? callInputs.map<Call>((inputs) => {
            const callsdata = contract.interface.encodeFunctionData(fragment, inputs)
            const calldata = contract.interface.decodeFunctionData(fragment, callsdata)
            return {
              addc: calldata,
              address: contract.address,
              callData: callsdata,
  ...
  // const results = useCallsData111(calls, options)
  if (methodName == 'quoteExactOutput' && calls.length > 0) {
    // debugger
  }
  const latestBlockNumber = useBlockNumber()

  return useMemo(() => {
    return calls.map((result) => toCallState111(result, contract?.interface, fragment, latestBlockNumber))
  }, [fragment, contract, latestBlockNumber])
}
